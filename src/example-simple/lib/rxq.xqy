...
declare variable $rxq:exclude-prefixes as xs:string* := ("rdf","sql","sem","plug-impl","xdmp", "hof", "impl", "plugin", "amped-info", "debug", "cts", "json", "amped-common", "rest", "rest-impl", "fn", "math", "xs", "prof", "sc", "dbg", "xml", "magick", "map", "xp", "rxq", "idecl", "xsi");
...
declare function rxq:resource-functions() as element(rxq:resource-functions){
  element rxq:resource-functions{
    for $f in xdmp:functions()[fn:prefix-from-QName(fn:function-name(.)) ne ''][fn:not(fn:prefix-from-QName(fn:function-name(.)) = $rxq:exclude-prefixes)]
      order by xdmp:annotation($f,xs:QName('rxq:path'))
      return
    if(xdmp:annotation($f,xs:QName('rxq:path')))  
    then
